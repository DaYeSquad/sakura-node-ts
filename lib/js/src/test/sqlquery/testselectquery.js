...
describe('测试查询语句只添加pk条件', () => {
    it('测试查询语句只添加pk条件', () => {
        let user = new User();
        user.initPk(111);
        const sql = new selectquery_1.SelectQuery().fromClass(User).select().pkwhere(user).build();
...
describe('测试查询语句添加pk条件和普通where条件', () => {
    it('测试查询语句添加pk条件和普通where条件', () => {
        let car = new Car();
        car.initPk(222);
        const sql = new selectquery_1.SelectQuery().fromClass(Car).select().pkwhere(car).where(`epid='111'`).build();
