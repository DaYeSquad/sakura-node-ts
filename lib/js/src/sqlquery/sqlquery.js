...
class SqlQuery {
    static getSqlInfoFromDefinition(model) {
        ...
        const sqlDefinitions = applicationcontext_1.applicationContext.findSqlFields(model.constructor);
    ...
    static valueAsStringByType(value, sqlType) {
        if (sqlType === model_1.SqlType.VARCHAR || sqlType === model_1.SqlType.TEXT) {
            value = `'${value}'`;
        }
