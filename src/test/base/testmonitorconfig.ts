...
describe("Test apiDoc to monitor config", () => {
  ...
  it("Test apiDoc to monitor config file", () => {
  ...
  it("Test apiDoc to monitor config object", () => {
    class UserController {
      static login(): void {}
      static getUsers(): void {}
    }
    ...
    const resultConfig: ProjectApiDescription = ApiDocContext.generateMonitorConfigObject({
      appId: 10086,
      host: "https://dev.gagogroup.cn/api/v4",
      timeInterval: 2,
      docs: [doc]
    });
    const resultJson = JSON.parse(JSON.stringify(resultConfig));
    const expectJson: ProjectApiDescription = require(path.resolve(__dirname, "../../../../../testdata/base/monitor-config.json"));

    chai.expect(expectJson.version).to.equal(resultJson.version);
    chai.expect(expectJson.appId).to.equal(resultJson.appId);
    chai.expect(JSON.stringify(expectJson.apis)).to.equal(JSON.stringify(resultJson.apis));
  });
});
